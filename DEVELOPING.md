...
**Gemini Omni**: generates one clip per call and always with sound, 3 to 10 seconds long, at its catalog-supported `global` location. The model id ends in `-preview`: it is a Preview model. Video output is billed per token; at the September 2026 list price that is roughly $0.03 per second at 360p, $0.10 at 720p, $0.15 at 1080p and $0.30 at 4K (see the official pricing page: https://cloud.google.com/gemini-enterprise-agent-platform/generative-ai/pricing). It is generated through `actions_lib/omni.py` via the Interactions API, and the `family` field of the catalog decides which transport `actions/generate_video.py` uses. The edit_video action sends an existing clip plus a prompt back to Omni and stores the edited clip; only models whose catalog entry lists edit_video can run it. In the app, the Edit button on a candidate runs edit_video with the candidate's clip and a prompt; it is shown only when the catalog lists a model with edit_video at a supported resolved location.
...
The project's “Use clip audio” choice is retained on each generated candidate, including Omni candidates: Omni always produces an audio track, but turning the choice off mutes it in previews and the final video.
