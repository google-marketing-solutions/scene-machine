...
def test_add_iam_binding_caches_project_iam_policy_across_calls(tmp_path):
  """add_iam_binding fetches get-iam-policy once per project instead of per role."""
  bin_dir = tmp_path / "bin"
  bin_dir.mkdir()
  calls_log = tmp_path / "gcloud_calls.log"
  gcloud_stub = bin_dir / "gcloud"
  sa_member = "serviceAccount:sm-runtime@p.iam.gserviceaccount.com"
  gcloud_stub.write_text(
      "#!/usr/bin/env bash\n"
      f'echo "$*" >> "{calls_log}"\n'
      'if [[ "$1 $2" == "projects get-iam-policy" ]]; then\n'
      f'  printf "roles/datastore.user\\t{sa_member}\\n"\n'
      f'  printf "roles/aiplatform.user\\t{sa_member}\\n"\n'
      "  exit 0\n"
      "fi\n"
      "exit 0\n",
      encoding="utf-8",
  ...
  libs_path = _REPO / "deploy" / "libs.sh"
  script = (
      "set -euo pipefail\n"
      f'source "{libs_path}"\n'
      f'add_iam_binding p --member="{sa_member}" --role="roles/datastore.user"\n'
      f'add_iam_binding p --member="{sa_member}" --role="roles/aiplatform.user"\n'
  )
  res = subprocess.run(
      ["bash", "-c", script],
      env=env,
  ...
  assert len(calls) == 1
  assert "projects get-iam-policy p" in calls[0]
